import { a as vi, r as re, H as Di, j as O, L as Ui, e as fn, C as bi, I as Gi, J as ge, i as Ee, M as Oe, N as Hi, O as Vi, p as Wi, R as $i, U as zi, V as Zi, W as Yi, X as Xi, Y as Ji, Z as Qi, $ as es, f as rs, a0 as ts, a1 as ze, a2 as ns, a3 as as, a4 as is, a5 as ss, k as _i, b as os, n as cs, a6 as us, a7 as ls, a8 as fs, c as ds, F as Fe, a9 as hs, aa as ps, B as ys } from "./index-DyzPd4zG.js";
import { C as gs } from "./customNumberKeyboard-Bej4bUDg.js";
const ms = "/assets/adminHomeWallet_dark-DiIyX2BO.png";
function vs({ scannedAddress: r, sparkInformation: t }) {
  try {
...
function bs({ reason: r }) {
  const t = vi(), e = re.useRef(null), n = re.useMemo(() => Di(Gi, "light"), []);
  return re.useEffect(() => {
    e.current && e.current.play();
  }, []), O.jsxs("div", { className: "errorWithPaymentComponent", children: [O.jsx(Ui, { className: "errorAnimation", lottieRef: e, animationData: n, loop: false, autoplay: true }), O.jsx(fn, { className: "errorText", textContent: "Error message" }), O.jsx("div", { className: "sendErrorContentContainer", children: O.jsx(fn, { styles: { textAlign: "center" }, textContent: String(r) }) }), O.jsx(bi, { textContent: "Continue", actionFunction: () => {
    t("/wallet");
  } })] });
}
...
function Ho() {
  if (pi) return zt;
  pi = 1;
  var r = no(), t = ao(), e = Ci(), n = go(), a = bo(), f = _o(), c = wo(), o = Ao(), i = Eo(), s = To(), l = qo(), h = on(), y = Po(), _ = Mo(), A = Fo(), g = Qt(), m = xi(), E = Do(), w = Le(), C = Go(), S = nn(), P = an(), R = 1, K = 2, T = 4, L = "[object Arguments]", W = "[object Array]", q = "[object Boolean]", Q = "[object Date]", oe = "[object Error]", te = "[object Function]", U = "[object GeneratorFunction]", le = "[object Map]", ce = "[object Number]", $ = "[object Object]", Y = "[object RegExp]", Te = "[object Set]", ne = "[object String]", me = "[object Symbol]", D = "[object WeakMap]", _e = "[object ArrayBuffer]", ue = "[object DataView]", qe = "[object Float32Array]", ve = "[object Float64Array]", we = "[object Int8Array]", Me = "[object Int16Array]", ee = "[object Int32Array]", G = "[object Uint8Array]", Ie = "[object Uint8ClampedArray]", Ce = "[object Uint16Array]", Se = "[object Uint32Array]", k = {};
  k[L] = k[W] = k[_e] = k[ue] = k[q] = k[Q] = k[qe] = k[ve] = k[we] = k[Me] = k[ee] = k[le] = k[ce] = k[$] = k[Y] = k[Te] = k[ne] = k[me] = k[G] = k[Ie] = k[Ce] = k[Se] = true, k[oe] = k[te] = k[D] = false;
  function fe(j, de, he, We, d, p) {
    var u, b = de & R, v = de & K, I = de & T;
    if (he && (u = d ? he(j, We, d, p) : he(j)), u !== void 0) return u;
    if (!w(j)) return j;
    var F = g(j);
    if (F) {
      if (u = y(j), !b) return c(j, u);
    } else {
      var x = h(j), X = x == te || x == U;
      if (m(j)) return f(j, b);
      if (x == $ || x == L || X && !d) {
        if (u = v || X ? {} : A(j), !b) return v ? i(j, a(u, j)) : o(j, n(u, j));
      } else {
        if (!k[x]) return d ? j : {};
        u = _(j, x, b);
      }
    ...
    var V = I ? v ? l : s : v ? P : S, B = F ? void 0 : V(j);
...
function Wo() {
  ...
  for (let d = 0, p = Object.keys(K); d < p.length; d++) {
    const u = p[d], b = K[p[d]].toString();
    T[b] = u;
  }
  const L = { payment_hash: ne, payment_secret: ne, description: me, payee_node_key: ne, purpose_commit_hash: Me, expire_time: U, min_final_cltv_expiry: U, fallback_address: _e, routing_info: we, feature_bits: ve }, W = { 1: (d) => $(d, true).toString("hex"), 16: (d) => $(d, true).toString("hex"), 13: (d) => $(d, true).toString("utf8"), 19: (d) => $(d, true).toString("hex"), 23: (d) => $(d, true).toString("hex"), 6: te, 24: te, 9: D, 3: ue, 5: qe }, q = "unknownTag";
  function Q(d) {
  ...
  function te(d) {
    return d.reverse().reduce((p, u, b) => p + u * Math.pow(32, b), 0);
  }
  function U(d, p) {
    const u = [];
  ...
  function ce(d, p, u) {
    let b = 0, v = 0;
    const I = (1 << u) - 1, F = [];
    for (let x = 0; x < d.length; ++x) for (b = b << p | d[x], v += p; v >= u; ) v -= u, F.push(b >> v & I);
    return v > 0 && F.push(b << u - v & I), F;
  }
  function $(d, p) {
  ...
  function D(d, p) {
    ...
    switch (u) {
      case 17:
        v = f.toBase58Check(b, p.pubKeyHash);
        break;
      case 18:
        v = f.toBase58Check(b, p.scriptHash);
        break;
      case 0:
      case 1:
        v = f.toBech32(b, u, p.bech32);
        break;
    }
    return { code: u, address: v, addressHash: b.toString("hex") };
  }
  function _e(d, p) {
  ...
  function ue(d) {
    const p = [];
    let u, b, v, I, F, x = $(d, true);
    for (; x.length > 0; ) u = x.slice(0, 33).toString("hex"), b = x.slice(33, 41).toString("hex"), v = parseInt(x.slice(41, 45).toString("hex"), 16), I = parseInt(x.slice(45, 49).toString("hex"), 16), F = parseInt(x.slice(49, 51).toString("hex"), 16), x = x.slice(51), p.push({ pubkey: u, short_channel_id: b, fee_base_msat: v, fee_proportional_millionths: I, cltv_expiry_delta: F });
    return p;
  }
  function qe(d) {
    const p = d.slice().reverse().map((b) => [!!(b & 1), !!(b & 2), !!(b & 4), !!(b & 8), !!(b & 16)]).reduce((b, v) => b.concat(v), []);
    for (; p.length < g.length * 2; ) p.push(false);
    const u = { word_length: d.length };
    if (g.forEach((b, v) => {
      u[b] = { required: p[v * 2], supported: p[v * 2 + 1] };
    }), p.length > g.length * 2) {
      const b = p.slice(g.length * 2);
      u.extra_bits = { start_bit: g.length * 2, bits: b, has_required: b.reduce((v, I, F) => F % 2 !== 0 ? v || false : v || I, false) };
  ...
  function ve(d) {
    ...
    for (g.forEach((b) => {
      u.push(!!(d[b] || {}).required), u.push(!!(d[b] || {}).supported);
    ...
    return p === void 0 && (p = Math.ceil(u.length / 5)), new Array(p).fill(0).map((b, v) => u[v * 5 + 4] << 4 | u[v * 5 + 3] << 3 | u[v * 5 + 2] << 2 | u[v * 5 + 1] << 1 | u[v * 5] << 0).reverse();
  }
  function we(d) {
    let p = n.from([]);
    return d.forEach((u) => {
  ...
  function ee(d, p) {
    const u = d.filter((v) => v.tagName === p);
  ...
  function Ie(d, p) {
    const u = {};
    if (Object.keys(d).sort().forEach((b) => {
      u[b] = d[b];
    }), p === true) {
      const b = "__tagsObject_cache";
      Object.defineProperty(u, "tagsObject", { get() {
        return this[b] || Object.defineProperty(this, b, { value: We(this.tags) }), this[b];
      } });
    }
    return u;
  }
  ...
  function Se(d) {
    ...
    const p = new a(d, 10), u = p.toString(10), b = u.length;
    let v, I;
    return b > 11 && /0{11}$/.test(u) ? (v = "", I = p.div(w).toString(10)) : b > 8 && /0{8}$/.test(u) ? (v = "m", I = p.div(C).toString(10)) : b > 5 && /0{5}$/.test(u) ? (v = "u", I = p.div(S).toString(10)) : b > 2 && /0{2}$/.test(u) ? (v = "n", I = p.div(P).toString(10)) : (v = "p", I = p.mul(R).toString(10)), I + v;
  }
  function k(d, p) {
    const u = fe(d, false);
    if (!u.mod(new a(1e3, 10)).eq(new a(0, 10))) throw new Error("Amount is outside of valid range");
    const b = u.div(new a(1e3, 10));
    return p ? b.toString() : b;
  }
  function fe(d, p) {
    let u, b;
    if (d.slice(-1).match(/^[munp]$/)) u = d.slice(-1), b = d.slice(0, -1);
    else {
      if (d.slice(-1).match(/^[^munp0-9]$/)) throw new Error("Not a valid multiplier for the amount");
      b = d;
    }
    if (!b.match(/^\d+$/)) throw new Error("Not a valid human readable amount");
    const v = new a(b, 10), I = u ? v.mul(w).div(m[u]) : v.mul(w);
    if (u === "p" && !v.mod(new a(10, 10)).eq(new a(0, 10)) || I.gt(E)) throw new Error("Amount is outside of valid range");
    return p ? I.toString() : I;
  }
  function j(d, p) {
    const u = c(d), b = Y(p);
    if (u.complete && u.paymentRequest) return u;
    if (b === void 0 || b.length !== 32 || !e.privateKeyVerify(b)) throw new Error("privateKey must be a 32 byte Buffer and valid private key");
    let v, I;
    if (G(u.tags, T[19]) && (I = Y(ee(u.tags, T[19]))), u.payeeNodeKey && (v = Y(u.payeeNodeKey)), v && I && !I.equals(v)) throw new Error("payee node key tag and payeeNodeKey attribute must match");
    v = I || v;
    const F = n.from(e.publicKeyCreate(b));
    if (v && !F.equals(v)) throw new Error("The private key given is not the private key of the node public key given");
    const x = t.decode(u.wordsTemp, Number.MAX_SAFE_INTEGER).words, X = n.concat([n.from(u.prefix, "utf8"), $(x)]), z = le(X), V = e.ecdsaSign(z, b);
  ...
  function de(d, p) {
    const u = c(d);
    p === void 0 && (p = true);
    const b = !(u.signature === void 0 || u.recoveryFlag === void 0);
    let v;
    if (u.network === void 0 && !b) u.network = o, v = o;
    else {
      if (u.network === void 0 && b) throw new Error("Need network for proper payment request reconstruction");
    ...
    if (u.timestamp === void 0 && !b) u.timestamp = Math.floor((/* @__PURE__ */ new Date()).getTime() / 1e3);
    else if (u.timestamp === void 0 && b) throw new Error("Need timestamp for proper payment request reconstruction");
    ...
    !G(u.tags, T[6]) && !b && p && u.tags.push({ tagName: T[6], data: h }), !G(u.tags, T[24]) && !b && p && u.tags.push({ tagName: T[24], data: y });
    let I, F;
    ...
    if (G(u.tags, T[9])) {
      ...
      if (z = M.address, X = M.addressHash, x = M.code, X === void 0 || x === void 0) {
        ...
        try {
          ...
          try {
            H = f.fromBase58Check(z), H.version === v.pubKeyHash ? x = 17 : H.version === v.scriptHash && (x = 18), X = H.hash;
          } catch {
            throw new Error("Fallback address type is unknown");
          }
        }
        if (N && !(N.version in v.validWitnessVersions)) throw new Error("Fallback address witness version is unknown");
        if (N && N.prefix !== v.bech32) throw new Error("Fallback address network type does not match payment request network type");
        if (H && H.version !== v.pubKeyHash && H.version !== v.scriptHash) throw new Error("Fallback address version (base58) is unknown or the network type is incorrect");
    ...
    J.forEach((M) => {
      ...
      if (b && N.push(q), N.indexOf(M.tagName) === -1) throw new Error("Unknown tag key: " + M.tagName);
      let H;
      if (M.tagName !== q) {
        ae.push(K[M.tagName]);
  ...
  function he(d, p) {
    ...
    const b = u.prefix;
    let v = u.words;
    const I = v.slice(-104), F = v.slice(0, -104);
    v = v.slice(0, -104);
    let x = $(I, true);
    const X = x.slice(-1)[0];
    if (x = x.slice(0, -1), !(X in [0, 1, 2, 3]) || x.length !== 64) throw new Error("Signature is missing or incorrect");
    let z = b.match(/^ln(\S+?)(\d*)([a-zA-Z]?)$/);
    if (z && !z[2] && (z = b.match(/^ln(\S+)$/)), !z) throw new Error("Not a proper lightning payment request");
    const V = z[1];
    let B;
    if (p) {
    ...
    const xe = te(v.slice(0, 7)), $e = new Date(xe * 1e3).toISOString();
    v = v.slice(7);
    const ie = [];
    let M, N, H, ye;
    for (; v.length > 0; ) {
      const Ae = v[0].toString();
      M = T[Ae] || q, N = W[Ae] || oe(Ae), v = v.slice(1), H = te(v.slice(0, 2)), v = v.slice(2), ye = v.slice(0, H), v = v.slice(H), ie.push({ tagName: M, data: N(ye, B) });
    }
    let Re, un;
    G(ie, T[6]) && (Re = xe + ee(ie, T[6]), un = new Date(Re * 1e3).toISOString());
    const Fi = n.concat([n.from(b, "utf8"), n.from(ce(F, 5, 8))]), Bi = le(Fi), ln = n.from(e.ecdsaRecover(x, X, Bi, true));
    ...
    let Ke = { paymentRequest: d, complete: true, prefix: b, wordsTemp: t.encode("temp", F.concat(I), Number.MAX_SAFE_INTEGER), network: B, satoshis: J, millisatoshis: ae, timestamp: xe, timestampString: $e, payeeNodeKey: ln.toString("hex"), signature: x.toString("hex"), recoveryFlag: X, tags: ie };
...
function uc() {
  ...
  (i == null ? void 0 : i.paymentNetwork) === "lightning" && i.type === "bolt11" && ((_a2 = i == null ? void 0 : i.data) == null ? void 0 : _a2.invoice.amountMsat), re.useEffect(() => {
    async function D() {
      var _a3, _b;
      if (vs({ scannedAddress: e, sparkInformation: W })) {
    ...
    if ($ && !g) {
      ...
      try {
        ...
        if (i.type === "liquid" && ue.didWork) {
          async function qe() {
            let ve = false, we = 0;
            for (; !ve && we < 10; ) we += 1, (await (await fetch(ps(void 0) + `/v2/swap/${i.boltzData.id}`)).json()).status === "invoice.settled" ? (ve = true, L("/confirm-page", { state: { for: "paymentsucceed", transaction: ue.response }, replace: true })) : await new Promise((G) => setTimeout(G, 5e3));
            ve || L("/confirm-page", { state: { for: "paymentFailed", transaction: { ...ue.response, details: { ...ue.response.details, error: "Unable to settle swap" } } }, replace: true });
          }
          qe();
          return;
        }
